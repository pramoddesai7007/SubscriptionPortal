...
export default function Login() {
  ...
  const handleLogin = async () => {
    try {
        ...
        if (username === 'sonam' && password === '@123456') {
          router.push('/signUp');
          return; // Stop further execution if redirected to /listCompany
        }
      // Authenticate the user
      const response = await axios.post('https://token-backend-beta.vercel.app/api/auth/loginOnline', { username, password });
      const token = response.data.token;

      // Save token to localStorage
      localStorage.setItem('OnlineAuthToken', token);
  ...
  return (
    <>
        ...
        <div className="py-12 font-sans">
          <div className="flex bg-white rounded-lg shadow-lg overflow-hidden mx-auto max-w-sm lg:max-w-4xl mt-7">
